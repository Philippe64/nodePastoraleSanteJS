...
function route(request,response) {
    ...
    logger.log("uriParsed ", util.inspect(uriParsed));
   ...
   logger.log("domain : ", domain, "  subdomain : ", subdomain);
    ...
    logger.log ("appliName" , appliName);
    var pathName = "";
    // check if 1st path of url  is nomApplication
    // if yes , remove nomApplication du pathName
    // on ne transporte pas la querystring
    if (appliName ==""){
        var urls = uriParsed.pathname.substr(1).split("\/");
